...
| 文件 | SHA-256 前缀 | 对应游戏 |
|---|---|---|
| `1ae7153ed25d.dll` | 1ae7153ed25d... | 当前工作区目标游戏（Sanoba Witch） |
| `9bd81f525ace.dll` | 9bd81f525ace... | 另一款含 Kanna/Natsume/Nozomi/Mei/Suzune 角色的游戏 |
...
两个 DLL 是**同一套加密插件框架针对不同游戏编译的配置变体**。框架代码完全复用，所有差异均为每游戏独立配置的加密材料：密码置换参数表、游戏唯一标识字符串（UNIQUE）、RSA 公钥（PUBKEY），以及档案名哈希种子。分析当前游戏时，应以 `1ae7153ed25d.dll` 为准；`9bd81f525ace.dll` 的配置数据不可混用。
